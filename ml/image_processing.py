...
tf.app.flags.DEFINE_integer('image_depth', 1,
                            """Provide square images of channels.""")
...
def decode_jpeg(image_buffer, scope=None):
  ...
  with tf.name_scope(values=[image_buffer], name=scope,
    ...
    image = tf.image.decode_jpeg(image_buffer, channels=FLAGS.image_depth)
...
def distort_image(image, height, width, bbox, thread_id=0, scope=None):
  ...
  # the entire image.
    ...
    # the third dimension.
    distorted_image.set_shape([height, width, FLAGS.image_depth])
    if not thread_id:
