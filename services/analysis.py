...
cascade_dir = os.path.realpath(os.path.join(os.curdir, 'services', 'cascades'))
...
class Analyzer:
    ...
    def analyze_erythema(self, erythema_img, result_data=None, visible=False):
        roi_size = erythema_img.shape[0] * erythema_img[1]
    ...
    def analyze_pore(self, pore_img, result_data=None, visible=False):
        roi_size = pore_img.shape[0] * pore_img.shape[1]
        ...
        score = 1 - (max(1, num_pore) / roi_size)
    ...
    def analyze_pigmentation(self, pigmentation_img, result_data=None, visible=False):
        roi_size = pigmentation_img.shape[0] * pigmentation_img[1]
...
class CascadeDetector:
    ...
    def detect_face(self, img, use_gray=False, visible=False):
        face_cascade = cv2.CascadeClassifier(os.path.join(cascade_dir, 'frontalFace.xml'))
        print(type(face_cascade), os.path.join(cascade_dir, 'frontalFace.xml'))
        overlay = img.copy()
    ...
    def detect_facial_feature(self, img, visible=False):
        face_cascade = cv2.CascadeClassifier(os.path.join(cascade_dir, 'frontalFace.xml'))
        eye_cascade = cv2.CascadeClassifier(os.path.join(cascade_dir, 'Eyes_cascade.xml'))
        nose_cascade = cv2.CascadeClassifier(os.path.join(cascade_dir, 'Nose_cascade.xml'))
        mouth_cascade = cv2.CascadeClassifier(os.path.join(cascade_dir, 'Mouth_cascade.xml'))
...
class LandmarkDetector:
    ...
    def get_landmarks(self, img):
        PREDICTIOR_PATH = os.path.join(cascade_dir,
